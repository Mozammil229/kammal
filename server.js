...
let memoryCache = {
    ...
    users: {
        ...
        "mozammil": { username: "mozammil", credits: 150, isPro: true, status: "active", deviceId: "HW-44109-MZ", customPrompt: "" }
    }
};
...
async function connectToDatabase() {
    ...
    try {
        ...
        console.warn("MongoDB Atlas connection warning:", e.message);
        return false;
    }
}
...
app.post('/api/admin/system-prompt', requireAdminAuth, async (req, res) => {
    ...
    if (cachedDb && mongoose.connection.readyState === 1) {
        await Config.findOneAndUpdate({ key: 'global_config' }, { systemPrompt: systemPrompt.trim() }, { upsert: true });
    }
...
app.post('/api/admin/model', requireAdminAuth, async (req, res) => {
    ...
    if (cachedDb && mongoose.connection.readyState === 1) {
        await Config.findOneAndUpdate({ key: 'global_config' }, { currentModel: model }, { upsert: true });
    }
...
app.post('/api/admin/test-prompt', requireAdminAuth, async (req, res) => {
    ...
    try {
        if (genAI) {
            const model = genAI.getGenerativeModel({ model: modelName, systemInstruction: prompt || conf.systemPrompt });
...
app.post('/api/admin/user-prompt', requireAdminAuth, async (req, res) => {
    ...
    if (cachedDb && mongoose.connection.readyState === 1) {
        await User.findOneAndUpdate({ username }, { $set: { customPrompt: promptText } }, { upsert: true });
    }
    if (memoryCache.users[username]) {
        memoryCache.users[username].customPrompt = promptText;
    }
    res.json({ success: true, message: promptText ? `VIP Prompt saved for ${username} ⭐` : `Reverted to Global Prompt.` });
});

// 3. User Ledger & HW Lock
app.get('/api/admin/users', requireAdminAuth, async (req, res) => {
    const search = (req.query.search || "").toLowerCase();
    if (cachedDb && mongoose.connection.readyState === 1) {
        let query = search ? {
            $or: [{ username: { $regex: search, $options: 'i' } }, { deviceId: { $regex: search, $options: 'i' } }]
        } : {};
        const users = await User.find(query).lean();
        return res.json({ success: true, count: users.length, users });
    }
    let list = Object.values(memoryCache.users);
    if (search) list = list.filter(u => u.username.toLowerCase().includes(search) || (u.deviceId && u.deviceId.toLowerCase().includes(search)));
...
app.post('/api/admin/maintenance', requireAdminAuth, async (req, res) => {
    ...
    if (cachedDb && mongoose.connection.readyState === 1) {
        await Config.findOneAndUpdate({ key: 'global_config' }, { maintenance: maintData }, { upsert: true });
    }
    memoryCache.maintenance = maintData;
    res.json({ success: true, message: enabled ? "App FROZEN globally." : "App UNFROZEN." });
});

// 5. Android App Chat & HW Check
app.post('/api/chat', async (req, res) => {
    ...
    if (username && cachedDb && mongoose.connection.readyState === 1) {
