...
const ADMIN_SECURITY_KEY = String(process.env.ADMIN_KEY || "").trim();
...
function requireAdminAuth(req, res, next) {
  const raw = req.headers["x-admin-key"] || req.query.adminKey || req.body?.adminKey || "";
  const key = String(raw).trim();
  if (!ADMIN_SECURITY_KEY) {
    return res.status(503).json({ success: false, error: "Admin key is not configured on the server." });
  }
